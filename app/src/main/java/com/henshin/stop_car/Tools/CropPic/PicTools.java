package com.henshin.stop_car.Tools.CropPic;
...
/**
 * Created by henshin on 2018/3/2.
 */
...
public class PicTools {
    ...
    public void startCamera() {
        ...
        if(Build.VERSION.SDK_INT >= 24) {
            photoUri = FileProvider.getUriForFile(context, "com.henshin.provider", file);
        } else {
        ...
        switch (requestCode) {
            ...
            case TAKE_PHOTO_PERMISSION_REQUEST_CODE:
                ...
                if(grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(context, "拍照权限被拒绝", Toast.LENGTH_SHORT).show();
                }
                break;
            // 打开相册选取：
            case WRITE_SDCARD_PERMISSION_REQUEST_CODE:
                if(grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(context, "读写内存卡内容权限被拒绝", Toast.LENGTH_SHORT).show();
                }
                break;
        }
    }
     ...
     */
    public void oncallResult(int requestCode, int resultCode, Intent data, ImageView pictureImageView) {
...
//                        file.delete(); // 选取完后删除照片
                        ...
                        Toast.makeText(context, "找不到照片", Toast.LENGTH_SHORT).show();
                    }
                    break;
            }
        }
