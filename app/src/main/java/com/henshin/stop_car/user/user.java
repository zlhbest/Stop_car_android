package com.henshin.stop_car.user;

/**
 * Created by henshin on 2018/2/24.
 */

public class user {
    private int aa001;//用户流水号
    private String aa002;//用户姓名
    private String aa003;//用户密码
    private String aa004;//性别
    private String aa005;//状态
    private float aa006;// 用户余额
    private String aa007;//Email
    private String aa008;//电话
    public void setAa001(int Aa001)
    {
        this.aa001 = Aa001;
    }
    public int getAa001()
    {
        return this.aa001;
    }
    public void setAa002(String Aa002)
    {
        this.aa002 = Aa002;
    }
    public String getAa002()
    {
        return this.aa002;
    }
    public void setAa003(String Aa003)
    {
        this.aa003 = Aa003;
    }
    public String getAa003()
    {
        return this.aa003;
    }
    public void setAa004(String Aa004)
    {
        this.aa004 = Aa004;
    }
    public String getAa004()
    {
        return this.aa004;
    }
    public void setAa005(String Aa005)
    {
        this.aa005 = Aa005;
    }
    public String getAa005()
    {
        return this.aa005;
    }
    public void setAa005(float Aa006)
    {
        this.aa006 = Aa006;
    }
    public float getAa006()
    {
        return this.aa006;
    }
    public void setAa007(String Aa007)
    {
        this.aa007 = Aa007;
    }
    public String getAa007()
    {
        return this.aa007;
    }
    public void setAa008(String Aa008)
    {
        this.aa008 = Aa008;
    }
    public String getAa008()
    {
        return this.aa008;
    }
}
